...
class DivideImage:
    ...
    def __generate_patches(self):
        ...
        for i in range(math.ceil((self.height - lap_len) / step)): # row
            for j in range(math.ceil((self.width - lap_len) / step)): # col
                ...
                if self.mask is None or self.mask[idx1:idx2, idy1:idy2].max() > 0:
                    img = self.img[idx1:idx2, idy1:idy2]
...
class BigImage(Dataset):
    def __init__(self,img_path=None,img=None, patch_sz=512, overlap=0, mask=None, scale=1):
        super(BigImage, self).__init__()
        if img_path!=None:
            self.img = DivideImage(cv2.imread(img_path,0), patch_sz, overlap, mask, scale)
        else:
            self.img = DivideImage(img, patch_sz, overlap, mask, scale)
...
# if __name__ == '__main__':
    # imgs_ref = BigImage('/home/zhangzb/code/mult_work/data/input/10_res_aligned/0001.png',
    #                      patch_sz=1024,overlap=10) 
    # h,w=imgs_ref.merge().shape
    # print(h,w,len(imgs_ref))
    # for i in range(len(imgs_ref)):
    #     patch_mov, idx_mov = imgs_ref[i]
    #     print(idx_mov,patch_mov.shape)
    #     cv2.imwrite(f'./test/{i:04d}.png',patch_mov)
