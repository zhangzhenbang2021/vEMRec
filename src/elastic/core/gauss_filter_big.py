...
def coords_grid(batch: int, ht: int, wd: int, device: torch.device) -> torch.Tensor:
    """
    Generate a coordinate grid for image batches.
    ...
    Returns:
        torch.Tensor: A tensor of shape (batch, 2, ht, wd) containing the coordinate grid.
    """
...
def mesh_grid(B: int, H: int, W: int) -> torch.Tensor:
    """
    Create a mesh grid for the given batch size and image dimensions.
    ...
    Returns:
        torch.Tensor: A tensor of shape (B, 2, H, W) containing the mesh grid.
    """
...
def norm_grid(v_grid: torch.Tensor) -> torch.Tensor:
    """
    Normalize a grid to the range [-1, 1].

    Args:
        v_grid (torch.Tensor): A grid of shape (B, 2, H, W).

    Returns:
        torch.Tensor: A normalized grid of shape (B, H, W, 2).
    """
    _, _, H, W = v_grid.size()
...
def get_gaussian(s: tuple, sigma: float = 1.0 / 8) -> torch.Tensor:
    ...
    temp[tuple(coords)] = 1
    gaussian_map = gaussian_filter(temp, sigmas, mode='constant', cval=0)
    gaussian_map /= np.max(gaussian_map)
    return torch.from_numpy(gaussian_map)
...
def flow_estimation(
    ...
    flow = flow_merge(flow_list, idx_list, args.patch_sz, args.height, args.width, sigma=1.0)
    return flow
...
def gauss_filter(model, r, L, args):
    ...
    def safe_write_image(path, img):
        '''Safely writes an image and handles errors.'''
    ...
    all_image_names = sorted([file for file in os.listdir(args.input_dir)], key=sort_by_number)
    ...
    with tqdm(range(start_i, len(img_names) + r - 1), bar_format=bar_format, desc=f"PID {os.getpid()}") as pbar:
        for i in pbar: 
            ...
            if not imgs_cur:
                imgs_cur = [BigImage(img_path=img_paths[i + k], 
                                     patch_sz=args.patch_sz, overlap=args.overlap) for k in range(-r, r + 1)]
            else:
                imgs_cur = imgs_cur[1:] + [BigImage(img_path=img_paths[i + r], 
