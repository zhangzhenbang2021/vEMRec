...
class MyNet(nn.Module):
    ...
    def forward(self, x):
        ...
        #     fLen = len(feature_maps)
        #     fig, axes = plt.subplots(fLen, 2, figsize=(24, 24))  # 创建子图
        #     for i, feature_map1 in enumerate(feature_maps): 
        #         for j in range(2):
        #             feature_map =np.uint8((255 * feature_map1[0, j, :, :]))  # 获取单个特征图
        #             axes[i, j].imshow(feature_map, cmap='gray')  # 显示特征图
        ...
        # # 可视化特征图
        # visualize_feature_maps([res1_up_crop.data.cpu().numpy(), res2_up_crop.data.cpu().numpy(), res3_up_crop.data.cpu().numpy(),
        #                         res4_up_crop.data.cpu().numpy(), res5_up_crop.data.cpu().numpy()])
        ...
        #     # 保存为 PNG 图像文件
        #     edge_image.save(os.path.join(args.output_dir, '{}.png'.format(name)))
        ...
        # def visualize_feature_maps(feature_maps):
        #     fLen = len(feature_maps)
        #     fig, axes = plt.subplots(fLen, 16, figsize=(24, 24))  # 创建子图
        #     for i, feature_map1 in enumerate(feature_maps): 
        #         for j in range(16):
        #             feature_map =np.uint8((255 * feature_map1[0, j, :, :]))  # 获取单个特征图
        #             axes[i, j].imshow(feature_map, cmap='gray')  # 显示特征图
        ...
        # # 可视化特征图
        # visualize_feature_maps([depth_res5c_crop2.data.cpu().numpy(), unet1a_up_crop.cpu().numpy(), normal_res5c_crop2.cpu().numpy(),
        #                         reflectance_res123_up2_crop2.cpu().numpy(), reflectance_unet1a_up_2_crop.cpu().numpy(),
        #                         illumination_res123_up2_crop2.cpu().numpy(), illumination_unet1a_up_2_crop.cpu().numpy()])
        ...
        # def visualize_feature_maps(feature_maps):
        #     fLen = len(feature_maps)
        #     fig, axes = plt.subplots(fLen, 1, figsize=(24, 24))  # 创建子图
        #     for i, feature_map1 in enumerate(feature_maps): 
        #             feature_map = np.uint8((feature_map1[0, 0]))
        #             axes[i].imshow(feature_map, cmap='gray')  # 显示特征图
        ...
        # # 可视化特征图
        # visualize_feature_maps([out_depth.cpu().numpy(), out_normal.cpu().numpy(), out_reflectance.cpu().numpy(),out_illumination.cpu().numpy()])
